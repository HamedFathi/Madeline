...
export const typeCategoryFinder = function(
    ...
    if (source.classes) {
        const result = source.classes.filter(x => x.directory === from.directory && x.file === from.file);
    ...
    if (source.enums) {
        const result = source.enums.filter(x => x.directory === from.directory && x.file === from.file);
        if (result && result.length > 0) {
    ...
    if (source.functions) {
        const result = source.functions.filter(x => x.directory === from.directory && x.file === from.file);
        if (result && result.length > 0) {
            return TypeCategory.Functions;
        }
    }
    if (source.interfaces) {
        const result = source.interfaces.filter(x => x.directory === from.directory && x.file === from.file);
        if (result && result.length > 0) {
            return TypeCategory.Interfaces;
        }
    }
    if (source.literals) {
        const result = source.literals.filter(x => x.directory === from.directory && x.file === from.file);
        if (result && result.length > 0) {
            return TypeCategory.Literals;
        }
    }
    if (source.typeAliases) {
        const result = source.typeAliases.filter(x => x.directory === from.directory && x.file === from.file);
        if (result && result.length > 0) {
            return TypeCategory.TypeAliases;
        }
    }
    if (source.variables) {
        const result = source.variables.filter(x => x.directory === from.directory && x.file === from.file);
        if (result && result.length > 0) {
